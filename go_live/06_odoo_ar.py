...
def captureDataFromApi(steps, table_name):
    ...
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/8. Opena AR/Complete Master/" +
              table_name + "_" + date_time.strftime("%m%d%y") + ".csv", index=False)


def rewrite_ar_report(file_name):
    df = pd.read_excel(
        "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/8. Opena AR/Download Odoo/" + file_name)
    ...
    df_final.to_csv("files/relations/ar/history/aged_receivable_transformed_" +
                    date_time.strftime("%m%d%y") + ".csv.gz", index=False, compression='gzip')
    df_final.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/8. Opena AR/Download Odoo/" +
...
def readArAndTransform(file_name):
    ...
    df_ar.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/8. Opena AR/Download Odoo/open_ar_output_" +
                 date_time.strftime("%m%d%y") + ".csv", index=False)
    df_ar.to_excel("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/8. Opena AR/Download Odoo/open_ar_output_" +
