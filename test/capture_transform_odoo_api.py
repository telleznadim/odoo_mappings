...
def captureDataFromApi():
    ...
    df.to_csv("files/csvs/from_api/customers2.csv",index=False)
...
def readContactsTransform():
    ...
    # df['website'] = np.where(df['website']=='False', '', df["website"])
    # df['street'] = np.where(df['street']=='False', '', df["street"])
    # df['street2'] = np.where(df['street2']=='False', '', df["street2"])
    # df['zip'] = np.where(df['zip']=='False', '', df["zip"])
    # df['city'] = np.where(df['city']=='False', '', df["city"])
    # df['email_normalized'] = np.where(df['email_normalized']=='False', '', df["email_normalized"])
    # df['mobile'] = np.where(df['mobile']=='False', '', df["mobile"])
    # df['phone_sanitized'] = np.where(df['phone_sanitized']=='False', '', df["phone_sanitized"])
    df['website'] = df['website'].str.replace("False","")
    df['street'] = df['street'].str.replace("False","")
    df['street2'] = df['street2'].str.replace("False","")
    df['zip'] = df['zip'].str.replace("False","")
    ...
    # df['mobile'] = pd.to_numeric(re.sub('[^0-9]','', df.mobile.astype(str)), errors='coerce')
    # print(df['mobile'])
    ...
    df.rename(columns={"id": "No.", "display_name": "Name", "website":"Home Page","credit_limit":"Credit Limit ($)","active":"Blocked", "street":"Address","street2":"Address 2","zip":"ZIP Code","city":"City","state_id":"State","country_id":"Country/Region Code", "mobile":"Mobile Phone No.","company_type":"Partner Type","email_normalized":"Email","commercial_company_name":"Name 2","phone_sanitized":"Phone No.","property_account_position_id":"SureTax© Exemption Code","x_studio_customer_type":"Customer Vertical","property_payment_term_id":"Payment Terms Code","user_id":"Salesperson Code","create_date":"Customer Since"}, inplace=True)
    ...
    # print(getColumns(df))
    df.to_excel("customers_output2.xlsx",index=False)
