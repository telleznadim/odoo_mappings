...
customers = pd.read_excel("files/to_bc_outputs/customers3_output.xlsx")
...
def getMapping(old_value,relation_csv):
    new_value = old_value
    if (new_value != "False"):
...
def readSaleOrderLinesTransform():
    ...
    df.rename(columns={"sequence": "Line No.","product_uom_qty":"Quantity","price_unit":"Unit Price","discount":"Line Discount %","x_studio_unit_cost":"Unit Cost"}, inplace=True)

    # # # Static Columns
    df["Type"] = ""
    
    df_columns = ["Document Type","Document No.","Line No.","Type","No.","Location Code","Quantity","Unit Price","Line Discount %","Unit Cost"]
    df = df.reindex(columns=df_columns)
    # print(getColumns(df))
    print(df)
