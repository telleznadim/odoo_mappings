...
# bc_ids = pd.read_csv("files/relations/items/bc_ids.csv")
# bc_ids = pd.read_excel("files/relations/items/bc_ids.xlsx")
# bc_ids = pd.read_excel(
#     "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/3. Items/Upload - Final Map/LSO item final Map to Upload 08.25.22 V1.xlsx", skiprows=2)
bc_ids = pd.read_excel(
    "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/3. Upload to BC/LSO_ItemsToCreate_SimpleMap_NaTe_12.02.22_V2.xlsx")
print(bc_ids)
vendor_ids = pd.read_csv("files/relations/vendors/ids.csv")
brands = pd.read_csv("files/relations/items/brands.csv")
product_category = pd.read_csv("files/relations/items/product_category.csv")
inv_posting_group = pd.read_csv("files/relations/items/inv_posting_group.csv")
gen_posting_group = pd.read_csv("files/relations/items/gen_posting_group.csv")
supplierinfo = pd.read_csv("files/relations/items/supplierinfo.csv")
...
def getSupplierInfoTable():
    ...
    records_count = models.execute_kw(
        db, uid, password, 'product.supplierinfo', 'search_count', [[]])
    print(records_count)
    ...
    for x in range(0, math.ceil(records_count/limit)):
        ...
        record = models.execute_kw(db, uid, password, 'product.supplierinfo', 'read', [
            ids], {'fields': ["id", "name"]})
        df1 = pd.DataFrame.from_dict(record)
        df = pd.concat([df, df1])

    df.rename(columns={"id": "old_value",
              "name": "new_value"}, inplace=True)
    df.to_csv("files/relations/items/supplierinfo.csv", index=False)
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/5. Complete Master/supplierinfo" +
              date_time.strftime("%m%d%y_%H%M%S") + ".csv")
...
def captureDataFromApi():
    ...
    records_count = models.execute_kw(
        db, uid, password, 'product.product', 'search_count', [[]])
    print(records_count)
    print(records_count/5000)
    limit = 5000
    df = pd.DataFrame()
    ...
    for x in range(0, math.ceil(records_count/limit)):
        ...
        record = models.execute_kw(
            db, uid, password, 'product.product', 'read', [ids])
        df1 = pd.DataFrame.from_dict(record)
        df = pd.concat([df, df1])

    df.to_csv("files/csvs/from_api/items.csv", index=False)
    df.to_csv("files/csvs/from_api/history/items_" + date_time.strftime(
        "%m%d%y_%H%M%S") + ".csv.gz", index=False, compression='gzip')
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/5. Complete Master/items" +
              date_time.strftime("%m%d%y_%H%M%S") + ".csv", index=False)
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/5. Complete Master/items_" +
...
def getItemMapping(old_value, relation_csv, default_value):
    new_value = default_value
    if (old_value != False):
        # print(old_value)
        find_new = relation_csv.loc[relation_csv["Oodo ID"]
                                    == old_value]['BC New Item No.']
        # print(find_new)
        if (not find_new.empty):
            new_value = relation_csv.loc[relation_csv["Oodo ID"]
                                         == old_value]['BC New Item No.'].values[0]
    return (new_value)
...
def getProductCategory(old_value):
    ...
    if (new_value != "False"):
        find_new = product_category.loc[product_category["old_value"]
                                        == new_value]['new_value']
        # print(find_new)
        if (not find_new.empty):
            new_value = product_category.loc[product_category["old_value"]
...
def getVendorNo(old_value):
    ...
    new_value = ""
    if (len(old_value) > 0):
        find_new = supplierinfo.loc[supplierinfo["old_value"]
...
def getBrand(old_value):
    ...
    if (new_value != "False"):
        ...
        if (not find_new.empty):
            new_value = brands.loc[brands["old_value"]
                                   == new_value]['new_value'].values[0]
    else:
        new_value = "ZZN"
    return (new_value)
...
def getInventoryPostingGroup(old_value):
    ...
    new_value = old_value[0]
    if (new_value != "False"):
        find_new = inv_posting_group.loc[inv_posting_group["old_value"]
                                         == new_value]['new_value']
        # print(find_new)
        if (not find_new.empty):
            new_value = inv_posting_group.loc[inv_posting_group["old_value"]
                                              == new_value]['new_value'].values[0]
    else:
        new_value = ""
...
def getGenProdPostingGroup(old_value):
    ...
    new_value = old_value[0]
    if (new_value != "False"):
        find_new = gen_posting_group.loc[gen_posting_group["old_value"]
                                         == new_value]['new_value']
        # print(find_new)
        if (not find_new.empty):
            new_value = gen_posting_group.loc[gen_posting_group["old_value"]
                                              == new_value]['new_value'].values[0]
    else:
        new_value = ""
...
def readItemsTransform():
    ...
    df = pd.read_csv("files/csvs/from_api/items.csv")
    ...
    # # Transform DATA
    print("Getting No.")
    df["No."] = df["id"].apply(getItemMapping, args=(bc_ids, "MISSING",))
    df["in_bc"] = df["id"].apply(getInBc, args=(bc_ids, False,))
    ...
    df["Inventory Posting Group"] = df["categ_id"].apply(
        getInventoryPostingGroup)
    print("Getting Gen. Prod. Posting Group")
    df["Gen. Prod. Posting Group"] = df["categ_id"].apply(
        getGenProdPostingGroup)
    df["Vendor No."] = df["seller_ids"].apply(
        getMapping, args=(vendor_ids, "False",))
    ...
    df.rename(columns={"uom_name": "Base Unit of Measure", "list_price": "Unit Price", "standard_price": "Last Direct Cost",
              "__last_update": "Last Date Modified", "uom_po_id": "Purch. Unit of Measure", "x_studio_brand": "Brand", "id": "Manufacturer Code"}, inplace=True)
    ...
    # df_columns = ["No.","Item Category Code","Vendor No.","Description","Base Unit of Measure","Unit Price","Last Direct Cost","Item Tracking Code","Description 2","Last Date Modified","Sales Unit of Measure","Purch. Unit of Measure","Brand","Unit Cost","Unit List Price","Legacy ID No.","Type","Tax Group Code","Country/Region of Origin Code","Purchasing Code","Country/Region Purchased Code","Vendor Item No.","Inventory Posting Group","Gen. Prod. Posting Group","Costing Method","Global Dimension 2 Code","Division","Blocked","Manufacturer Code","Manufacturer Item No.","in_bc"]
    df_columns = ["No.", "Item Category Code", "Vendor No.", "Description", "Base Unit of Measure", "Unit Price", "Last Direct Cost", "Item Tracking Code", "Description 2", "Last Date Modified", "Sales Unit of Measure", "Purch. Unit of Measure", "Brand", "Unit Cost", "Unit List Price", "Legacy ID No.", "Type",
                  "Tax Group Code", "Country/Region of Origin Code", "Purchasing Code", "Country/Region Purchased Code", "Vendor Item No.", "Inventory Posting Group", "Gen. Prod. Posting Group", "Costing Method", "Global Dimension 2 Code", "Division", "Blocked", "Manufacturer Code", "Manufacturer Item No.", "in_bc", "product_tmpl_id", "seller_ids"]

    df = df.reindex(columns=df_columns)
    # print(getColumns(df))
    print(df)
    df2 = df[df['in_bc']]
    df2 = df2.filter(["No.", "Manufacturer Code",
                     "Manufacturer Item No."], axis=1)
    df_columns = ["No.", "Item Category Code", "Vendor No.", "Description", "Base Unit of Measure", "Unit Price", "Last Direct Cost", "Item Tracking Code", "Description 2", "Last Date Modified", "Sales Unit of Measure", "Purch. Unit of Measure", "Brand", "Unit Cost", "Unit List Price", "Legacy ID No.", "Type",
                  "Tax Group Code", "Country/Region of Origin Code", "Purchasing Code", "Country/Region Purchased Code", "Vendor Item No.", "Inventory Posting Group", "Gen. Prod. Posting Group", "Costing Method", "Global Dimension 2 Code", "Division", "Blocked", "Manufacturer Code", "Manufacturer Item No.", "in_bc", "product_tmpl_id", "seller_ids"]
    ...
    df_final.to_csv("files/to_bc_outputs/history/items_output_" +
                    date_time.strftime("%m%d%y_%H%M%S") + ".csv.gz", index=False, compression='gzip')
    df_final.to_excel("files/to_bc_outputs/items_output.xlsx", index=False)
    df_final.to_csv(
        "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/1. Download from Odoo/items_output" +
        date_time.strftime("%m%d%y_%H%M%S") + ".csv", index=False)
    df_final.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/1. Download from Odoo/items_output_" +
                    date_time.strftime("%m%d%y_%H%M%S") + ".csv.gz", index=False, compression='gzip')
    df_final.to_excel(
        "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/3. Items/1. Download from Odoo/items_output" +
        date_time.strftime("%m%d%y_%H%M%S") + ".xlsx", index=False)
