...
def captureDataFromApi():
    ...
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/2. Vendors/5. Complete Master/vendors_" +
...
def modifyIdsRelation(df_new):
    ...
    df_vendor_ids_final = pd.concat([vendor_ids, df_new])
    print("vendor ids final", df_vendor_ids_final)
    df_vendor_ids_final.to_csv(
...
def readContactsTransform():
    ...
    df["No."] = df["id"].apply(getMapping, args=(vendor_ids, "False",))
    print(df["No."])
    ...
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/2. Vendors/1. Download from Odoo/vendors_lso_" +
              date_time.strftime("%m%d%y") + ".csv", index=False)
    df.to_excel("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live/2. Vendors/1. Download from Odoo/vendors_lso_" +
