...
def captureDataFromApi():
    ...
    df.to_csv("files/csvs/from_api/history/customers_" + date_time.strftime("%m%d%y") + ".csv.gz",index=False, compression='gzip')
    df.to_csv("files/csvs/from_api/customers.csv",index=False)
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/1. Customers/Complete Master File/customers_" + date_time.strftime("%m%d%y") + ".csv", index=False)
...
def getCustomerVertical(customer_type):
    ...
    if (customer_type):
        ...
        customer_vertical = cstvrt.loc[cstvrt["odoo_id"] == customer_type]['bc_cstvrt'].values[0]
...
def getUserId(user_id):
    ...
    if (user_id):
        user_id = user_id[0]
        bc_sale_person = salesperson.loc[salesperson["odoo_user_id"] == user_id]['bc_user_id'].values[0]
...
def defineIds(customer_id):
    return("LSOC" + str(customer_id).zfill(6))
...
def readContactsTransform():
    ...
    df.rename(columns={ "display_name": "Name", "website":"Home Page","credit_limit":"Credit Limit ($)","active":"Blocked", "street":"Address","street2":"Address 2","zip":"ZIP Code","city":"City","state_id":"State","country_id":"Country/Region Code", "mobile":"Mobile Phone No.","company_type":"Partner Type","email_normalized":"Email","commercial_company_name":"Name 2","phone_sanitized":"Phone No.","property_account_position_id":"SureTax© Exemption Code","x_studio_customer_type":"Customer Vertical","property_payment_term_id":"Payment Terms Code","user_id":"Salesperson Code","create_date":"Customer Since","id":"odoo_id"}, inplace=True)
    ...
    df.to_csv("files/to_bc_outputs/history/customers_output_" + date_time.strftime("%m%d%y_%H%M%S") + ".csvs.gz",index=False, compression='gzip')
    df.to_csv("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/1. Customers/Download Odoo/customers.csv", index=False)
    df.to_excel("/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/11-LSO/Go Live Simulation/1. Customers/Download Odoo/customers.xlsx", index=False)
...
def parentIdFalse(parent_true):
    ...
    print(df_false)
    print(df_true)
    df_true.to_csv("files/csvs/from_api/customers_parent_id_not_false.csv",index=False)
    df_false.to_csv("files/csvs/from_api/customers_parent_id_false.csv",index=False)
    if(parent_true):
        return(df_false)
    else:
        return(df_true)
